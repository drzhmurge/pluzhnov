...
class N2SmartSliderFeatureSlideBackground {
    ...
    private function renderBackgroundVideo($slide) {
		...
		if ($slide->parameters->get('backgroundVideoReset', 0)) {
			N2JS::addInline('n2ss.ready(' . $slide->getSlider()->sliderId . ', function(slider, sliderElement) {
				sliderElement.on("mainAnimationComplete", 
					function(e, animation, previousSlideIndex, currentSlideIndex) {
						if(previousSlideIndex == ' . $slide->index . ' && jQuery("#n2-ss-' . $slide->getSlider()->sliderId . '").find(".n2-ss-slide-background-video")[previousSlideIndex]){
							jQuery("#n2-ss-' . $slide->getSlider()->sliderId . '").find(".n2-ss-slide-background-video")[previousSlideIndex].currentTime = 0;
						}
					});
			});');
		}
