    ...
    <?php endif; ?>
		...
		if ( is_user_logged_in() ) {
            ...
            if ( function_exists( "messages_get_unread_count" ) ) {
				...
				<a id="user-messages" class="notification-link" href="<?php echo esc_url( bp_core_get_user_domain( $user_id ) . bp_get_messages_slug() ) ?>">
			...
			if ( buddyboss_is_bp_active() && bp_is_active( 'notifications' ) ):
				...
				if ( function_exists( 'buddyboss_notification_bp_members_shortcode_bar_notifications_menu' ) ) {
					...
					<div class="header-notifications all-notifications">
						<a class="notification-link" href="<?php if ( $menu_link ) { echo $menu_link; } ?>">
			...
			<?php if ( buddyboss_is_bp_active() ) { ?>
				<div class="balance-profile"><div class="balance">Корзинок:	<?php echo do_shortcode( '[mycred_my_balance]' ); ?></div></div>
				<!--Account details -->
				<div class="header-account-login">
					...
					<div class="pop user-pop-links">
						<div class="pop-inner">
							...
							<span class="logout">
								<a href="<?php echo wp_logout_url(); ?>"><?php _e( 'Выйти', 'boss' ); ?></a>
							</span>
						</div>
					</div>
			...
			<?php if ( !is_user_logged_in() && buddyboss_is_bp_active() ) : ?>
                ...
                <?php } ?>
                    <?php if ( buddyboss_is_bp_active() && bp_get_signup_allowed() ) : ?>
                        <a href="<?php echo bp_get_signup_page(); ?>" class="register screen-reader-shortcut"><?php _e( 'Регистрация', 'boss' ); ?></a>
                    <?php endif; ?>

                    <a href="<?php echo wp_login_url(); ?>" class="login"><?php _e( 'Вход', 'boss' ); ?></a>
                <?php if( '2' == boss_get_option('boss_header') ){ ?>
                </div>
                <?php } ?>
